  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 160px 160px', gap: 16, alignItems: 'center', marginBottom: 20 }}>
        ...
        <select value={crop} onChange={(e) => setCrop(e.target.value)} style={{ padding: '10px 14px', borderRadius: 10, border: '1px solid #ddd' }}>
          {CROP_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}
        </select>
        <select value={category} onChange={(e) => setCategory(e.target.value)} style={{ padding: '10px 14px', borderRadius: 10, border: '1px solid #ddd' }}>
          {CATEGORY_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}
        </select>
      </div>

      {loading ? <div style={{ color: '#666' }}>Loading articles...</div> : (
        <div style={{ display: 'grid', gap: 18 }}>
          {filtered.length === 0 ? <div style={{ color: '#888' }}>No articles found.</div> : filtered.map((article) => (
            <div key={article._id} style={{ background: '#fff', borderRadius: 20, padding: 24, boxShadow: '0 16px 40px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
                ...
                <div style={{ textAlign: 'right', color: '#999', fontSize: 12 }}>{new Date(article.createdAt).toLocaleDateString('en-GB')}</div>
              </div>
              <p style={{ color: '#555', lineHeight: 1.8, marginBottom: 0 }}>{article.content}</p>
            </div>
          ))}
        </div>
      )}
