  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 20 }}>
        ...
        <select value={crop} onChange={(e) => setCrop(e.target.value)} style={{ padding: '10px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.12)', background: '#122916', color: '#e6f6ea', minWidth: 180 }}>
          {CROPS.map((option) => <option key={option} value={option}>{option}</option>)}
        </select>
      </div>

      {loading ? <div style={{ color: '#9fbfa8' }}>Loading advisories...</div> : (
        filtered.length === 0 ? <div style={{ color: '#9fbfa8' }}>No advisories match this filter.</div> : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 18 }}>
            {filtered.map((advisory) => (
              <div key={advisory._id} style={{ background: '#0f231a', borderRadius: 18, padding: 22, boxShadow: '0 14px 30px rgba(0,0,0,0.16)', border: '1px solid rgba(255,255,255,0.08)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
                  <div><strong style={{ fontSize: 16, color: '#e6f6ea' }}>{advisory.activity}</strong><div style={{ color: '#9fbfa8', marginTop: 4 }}>{advisory.crop}</div></div>
                  <Chip color="blue">{new Date(advisory.recommendedDate).toLocaleDateString('en-GB')}</Chip>
                </div>
                <p style={{ color: '#cfd9c8', lineHeight: 1.7 }}>{advisory.description}</p>
                {advisory.instructions && <div style={{ marginTop: 14 }}><strong style={{ color: '#e6f6ea' }}>Instructions:</strong><p style={{ margin: '8px 0 0', color: '#cfd9c8' }}>{advisory.instructions}</p></div>}
              </div>
            ))}
          </div>
        )
