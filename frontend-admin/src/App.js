...
function DashboardPage() {
  ...
  return (
    <div>
      {/* Hero */}
      <div style={{ background: 'linear-gradient(135deg, #f4faf0 0%, #e7f5e4 55%, #d0e8cc 100%)', borderRadius: 24, padding: '28px 32px', marginBottom: 28, color: '#1b5e20', border: '1px solid #c8e6c9', boxShadow: '0 16px 40px rgba(46, 125, 50, 0.10)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
          <Logo size={58} />
          <div>
            <div style={{ fontSize: 13, color: '#2e7d32', marginBottom: 4, fontWeight: 700 }}>{greeting},</div>
            <div style={{ fontSize: 24, fontWeight: 800, marginBottom: 2 }}>{user?.fullName}</div>
            <div style={{ fontSize: 13, color: '#4e7c4b' }}>
        ...
        <div style={{ marginTop: 8, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          {[
            ...
            <div key={s.label} style={{ background: 'rgba(255,255,255,0.75)', borderRadius: 10, padding: '7px 14px', fontSize: 12, border: '1px solid #dceedd' }}>
...
function QuickCard({ icon, label, desc, page, color }) {
  ...
  return (
    <div
      ...
      style={{ background: '#fff', borderRadius: 16, padding: 20, cursor: 'pointer', transition: 'all 0.15s', boxShadow: hover ? `0 10px 28px ${color}22` : '0 6px 18px rgba(0,0,0,0.06)', border: '1px solid #e9efe8', borderTop: `3px solid ${color}`, transform: hover ? 'translateY(-2px)' : 'none' }}>
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: "'Segoe UI', system-ui, sans-serif", color: '#1a1a1a' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {/* Topbar */}
        <div style={{ background: '#f8fbf8', padding: '14px 28px', borderBottom: '1px solid #e5efe4', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
